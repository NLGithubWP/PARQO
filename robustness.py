...
parser.add_argument('--db', type=str, default='imdbload', help='stats or imdbload')
parser.add_argument('--query_id', type=str, required=False, help='query id in job, e.g. 2a', default="17a")
...
if args.inst_id == 0:
    ...
    ON_SAMPLE = 'on-base/' + db_name+"base" + '/'
    args.inst_id = None
...
# this includes the CE of both base and join
...
# f = open(f'log/{ON_SAMPLE}imdbload_{query_id}++basicinfo.txt', 'w')
...
def get_plan_list(sensitive_rels):
    ...
    if args.inst_id != None:
        ...
        if db_name == 'imdbload':  # use the plan generated by full err profile (no train-test split)
            file = './plan/' + ON_SAMPLE + 'saved-tmp_plan_dict_' + db_name+"base" + '_' + query_id + cal_sen_method + '.txt'
            ...
            if save_samples and args.method == 'sobol':
                if new_sel_tuple not in sample_to_penalty_dict:
                    ...
                    probability_ = cal_prob_of_sample(samples=[new_sel_tuple],
                                                      sensitive_rels=list(
                                                          err_files_dict[get_pure_q_id(query_id, db_name)].keys()),
                                                      est_card=est_card,
                                                      raw_card=raw_card,
...
##### 5. RQO entrance #####
def rqo():
    log_file_path = './log/' + ON_SAMPLE + db_name + '_' + query_id + '_' + str(tolerance) + cal_sen_method + '.log'
    log_dir = os.path.dirname(log_file_path)
    if os.path.exists(log_dir):
        shutil.rmtree(log_dir)
    os.makedirs(log_dir, exist_ok=True)
    logging.basicConfig(filename=log_file_path, level=logging.INFO)
    logging.info("Log folder reset, and logging started.")
    ...
    if args.exe:
        ...
        default_plan_latency = round(
            get_real_latency(db_name, sql, times=exe_times, limit_time=1200000, output_plan=True), 1)
        ...
        recentered_opt_plan_latency = round(
            get_real_latency(db_name, sql, hint=recentered_opt_plan, times=exe_times, limit_time=1200000,
                             output_plan=True), 1)
...
def pqo_query_subset(N=100):
    ...
    for q_id, optimized_query in enumerate(optimized_queries_dict.values()):
        ...
        with open(f'query/join-order-benchmark/{get_pure_q_id(query_id, db_name)}-{q_id}-b{args.b}.sql',
                  'w') as sql_file:
...
def pqo(N=100):
    ...
    cursor = conn.cursor()
    logging.basicConfig(
        filename='./log/' + ON_SAMPLE + db_name + '_' + query_id + '_pqo_' + str(tolerance) + cal_sen_method + '.log',
        level=logging.INFO)
    ...
    for sql_id, para_sql in enumerate(sql_template):
        ...
        rob_latency = round(
            get_real_latency(db_name, para_sql, hint=cur_plan_list[rob_plan_id], times=3, limit_time=100000), 1)
